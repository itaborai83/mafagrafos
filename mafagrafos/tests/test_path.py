...
class TestPath(unittest.TestCase):
    ...
    def test_it_prepends_a_segment_to_an_empty_path(self):
        segment = Segment(from_label='B', to_label='A', pct=0.5, min_t=1, max_t=10)
        expected = Segment(from_label='B', to_label='A', pct=0.5, min_t=1, max_t=10)
    ...
    def test_it_prepends_a_segment_to_an_non_empty_path(self):
        segment = Segment(from_label='B', to_label='A', pct=0.5, min_t=1, max_t=10)
        self.path.push_segment(segment)
        segment = Segment(from_label='C', to_label='B', pct=0.25, min_t=1, max_t=10)
        expected = Segment(from_label='C', to_label='B', pct=0.25, min_t=1, max_t=10)
        self.path.push_segment(segment)
    ...
    def test_it_pops_a_segment(self):
        ...
        segment = Segment(from_label='D', to_label='C', pct=0.125, min_t=1, max_t=10)
        self.path.push_segment(segment)
    ...
    def test_it_fails_to_push_a_disconnected_segment(self):
        segment = Segment(from_label='B', to_label='A', pct=0.5, min_t=1, max_t=10)
        self.path.push_segment(segment)
        segment = Segment(from_label='D', to_label='C', pct=0.25, min_t=1, max_t=10)
        with self.assertRaises(AssertionError):
            self.path.push_segment(segment)
